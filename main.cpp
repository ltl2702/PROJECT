...
int main(int argc, char* argv[]){
	...
	// Set up menu options
	Text text_menu[NumMenu];
	bool is_selected[NumMenu] = {false,false,false}; //các trạng thái của tùy chọn

   
	bool is_quit_menu = false;

	const char* menu_option_text[NumMenu] = {"PLAY GAME", "EXIT", "HOW TO PLAY"};
    ...
    while (!is_quit_menu)
    {
        ...
        SDL_RenderClear(g_screen); //clear lại màn hình
		...
		//bấm chuột
            ...
            break;
			  ...
			  case SDL_KEYDOWN:
                if (g_event.key.keysym.sym == SDLK_ESCAPE) //esc
                {
                    is_quit_menu = true;
                    is_quit = true;
                }
	...
	while (!is_quit) {
		  ...
		  while (SDL_PollEvent(&g_event) != 0) {
			...
			character.HandleInputAction(g_event, g_screen); //xử lí di chuyển
		  }
		  ...
		  character.SetMapXY(map_data.start_x_, map_data.start_y_); //thiết lập tọa độ nhân vật trên map
		  character.DoPlayer(map_data); //xử lí vận tốc..
		  character.Show(g_screen);

		  game_map.SetMap(map_data); //cập nhật vị trí mới trên start_x, start_y
		  ...
		  if(character.GetMoneyCount() == 100) GameWin();
